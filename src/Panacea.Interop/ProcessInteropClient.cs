...
{
    public class ProcessInteropClient:IDisposable
    {
        ...
        protected ProcessInteropClient()
        {

        }

        public ProcessInteropClient(Stream stream)
        {
            this.stream = stream;
        }
        ...
        private void Listen()
        {
            ...
            try
            {
                while (!_stopped)
                {
                    ...
                    OnMessageReceived(JsonSerializer.DeserializeFromString<Message>(text.Trim('\0')));
                }
            }
            catch (Exception ex)
            {
        ...
        public object[] Call(string uri, int milliSecondsTimeout, params object[] args)
        {
            ...
            return new object[] { false.ToString() };
        }
        ...
        protected virtual void OnClose()
        {
            lock (_closedLock)
            {
                ...
                foreach (var task in keys)
                {
                    ...
                    Debug.WriteLine("clearing task: " + task.ToString());
                }
                _tasks.Clear();
                if (_closed) return;
